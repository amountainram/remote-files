...
async fn run() -> Result<()> {
    ...
    match command {
        ...
        Commands::Upload { src, dest, confirm } => {
            let client: Client = util::get_config(
                profile.as_deref(),
                cli_state.get().current.as_deref(),
                &cfg.get().buckets,
            ...
            let (fd, file_path) = {
                ...
                tokio::fs::metadata(src)
                    ...
                    .and_then(|metadata| async move {
                        if metadata.is_file() {
                            let path = tokio::fs::canonicalize(src)
                                .await
                                .with_context(|| "retrieving file canonical path")?;
                            Ok((
                                tokio::fs::File::open(path.as_path())
                                    .await
                                    .with_context(|| "opening file descriptor")?,
                                path,
                            ))
                        } else {
                            Err(anyhow!("path '{}' is not a file", src.to_string_lossy()))
                        }
                    })
                    .await
                    .with_context(|| "while checking if file exists")
            }?;

            if !confirm {
                println!("{}", "dry run execution".bold().yellow());
                println!();
                println!(
                    "{} {} {} {} {} {}",
                    "will upload file".green(),
                    file_path.to_string_lossy().bold().cyan(),
                    "to remote profile".green(),
                    client.name().bold().cyan(),
                    "at location".green(),
                    dest.to_string().bold().cyan()
                );
            } else {
                unimplemented!()
            }
